...
export const Projects: React.FC = () => {
  return (
    <section id="projects" data-theme-bg="#0A0A0A" className="relative bg-[#0A0A0A] py-36 px-6 md:px-12 lg:px-20 text-white transition-colors duration-500">
      <div className="max-w-7xl mx-auto relative z-10">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          <div className="flex flex-col space-y-3">
            <div className="flex items-center gap-4">
              <span className="font-mono text-[10px] text-white/20 tracking-widest uppercase">05 /</span>
              <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-[#C5F52A]/70">Featured Showcase</span>
            </div>
            <TextReveal
              as="h2"
            ...
            >
              ...
              <div className={`lg:col-span-7 ${idx % 2 === 1 ? 'lg:order-2' : 'lg:order-1'}`}>
                <a
                  ...
                  className="block relative w-full aspect-[16/10] rounded-2xl bg-[#0A0A0A] border border-white/[0.08] overflow-hidden hover:scale-[1.01] transition-transform duration-500 shadow-2xl cursor-pointer group"
                  data-cursor="Visit"
                >
                  {/* Browser chrome bar */}
              ...
              <div className={`lg:col-span-5 space-y-5 ${idx % 2 === 1 ? 'lg:order-1' : 'lg:order-2'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono text-xs text-white/30 uppercase tracking-widest">Case Study</span>
                </div>

                <h3 className="font-section-heading text-3xl md:text-4xl font-bold text-white hover:text-[#C5F52A] transition-colors duration-300">
                ...
                {/* Impact — single clean callout, not duplicated */}
                <div className="flex items-center gap-3">
                  <span className="h-px w-4 bg-[#C5F52A]/50" />
                  <span className="font-body-ui text-sm font-semibold text-[#C5F52A]">{project.impact}</span>
                </div>

                {/* Tech tags — plain text with separators */}
